...
const WORKD_CONFIG_DIR = "/etc/workd"
...
// Set up `/etc/workd/config.json`.
func generateWorkdConfig(cr CommandRunner, insecureRegistries []string) error {
	...
	cmd := fmt.Sprintf(
		"sudo mkdir -p %s && printf %%s \"%s\" | base64 -d | sudo tee %s",
		WORKD_CONFIG_DIR,
		base64.StdEncoding.EncodeToString(json),
		path.Join(WORKD_CONFIG_DIR, "config.json"),
	)
	c := exec.Command("/bin/bash", "-c", cmd)
	if _, err := cr.RunCmd(c); err != nil {
		return errors.Wrap(err, "unable to generate workd config")
	}
	return nil
}
